...
urlpatterns = [
    ...
    path('outline/<uuid:outline_uuid>/script/', views.generate_full_script, name='generate-script'),
    
    # Outline CRUD
    path('outlines/', views.ScriptOutlineListView.as_view(), name='outline-list'),
    path('outlines/<uuid:uuid>/', views.ScriptOutlineDetailView.as_view(), name='outline-detail'),
    
    # Script CRUD
    path('scripts/', views.FullScriptListView.as_view(), name='full-script-list'),
    path('scripts/<uuid:uuid>/', views.FullScriptDetailView.as_view(), name='full-script-detail'),
]
