...
function StatCard({
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-5 flex flex-col gap-3">
      ...
      {trend && (
        <div className={`flex items-center gap-1 text-[13px] font-medium ${trendNeutral ? 'text-muted-foreground' : 'text-success'}`}>
          {trendNeutral ? <Minus className="h-3.5 w-3.5" /> : <TrendingUp className="h-3.5 w-3.5" />}
          {trend}
        </div>
      )}
    </div>
      ...
      <div className="p-7 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <StatCard
            ...
            trend={totalScripts + totalOutlines > 0 ? `+${totalScripts + totalOutlines} total` : undefined}
            icon={Sparkles}
          />
          <StatCard
                ...
                return (
                  <div key={t.name} className="flex items-start gap-4 rounded-md bg-secondary p-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <p className="text-[13px] font-medium leading-relaxed text-muted-foreground">{t.desc}</p>
                      <Link
                        href="/new-script"
